...
Expecting one of '${allowedValues.join("', '")}'`);
  ...
  _registerOption(option) {
    ...
    if (matchingOption) {
      const matchingFlag = option.long && this._findOption(option.long)
        ? option.long
        : option.short;
      throw new Error(
...
- either make a new Command for each call to parse, or stop storing options as properties`,
 ...
 - ${executableDirMessage}`;
  ...
  _conflictingOption(option, conflictingOption) {
    ...
    const message = `error: ${getErrorMessage(option)} cannot be used with ${
      getErrorMessage(conflictingOption)
    }`;
...
export { Command, useColor }; // exporting useColor for tests
